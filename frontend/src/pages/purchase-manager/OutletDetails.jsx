...
const OutletDetails = () => {
  ...
  const handleAllocate = () => {
    // If today's request is already packed, do not allow creating another one
    if (packedTodayRequest) {
      setAlert({
        type: 'warning',
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end lg:items-center justify-center p-0 lg:p-4">
          <div className="bg-card border-2 border-border rounded-t-2xl lg:rounded-xl p-5 lg:p-6 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4">
              ...
              <div
                ref={allocationRowsScrollRef}
                className="overflow-x-auto overflow-y-auto max-h-[42vh] scroll-smooth overscroll-contain border-2 border-border rounded-xl"
              >
                <table className="w-full min-w-[500px]">
                  ...
                  <tbody>
                    {allocationRows.map((row, index) => (
                      <tr
                        key={row.id || index}
                        className={`border-b border-border hover:bg-accent/5 animate-row-in ${selectedRows.has(index) ? 'bg-accent/10' : ''}`}
                      >
                          ...
                          <div className="min-w-[180px]">
                            ...
                            {openDropdownRow === index && ReactDOM.createPortal(
                              <div
                                ...
                                style={{ ...dropdownPosition, minWidth: 280 }}
                              >
                                <input

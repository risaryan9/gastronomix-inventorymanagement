...
const DispatchExecutiveDashboard = () => {
  ...
  useEffect(() => {
    if (!isPlanModalOpen || !cloudKitchenId) {
      setBatchUnitCosts({})
      return
    }
    const finishedIds = materials
    ...
    ;(async () => {
      try {
        ...
        console.error('Failed to load batch unit costs for dispatch plan:', err)
  ...
  }, [isPlanModalOpen, cloudKitchenId, materials])
                ...
                <div>
                  ...
                  {hasTodayPlan && !todayPlanIsLocked && (
                    <p className="text-xs lg:text-sm text-yellow-500 mt-2">
                      A draft for today already exists. Use the button below to update it until the kitchen locks the plan.
                    </p>
                  )}
                  {hasTodayPlan && todayPlanIsLocked && (
                    <p className="text-xs lg:text-sm text-yellow-500 mt-2">

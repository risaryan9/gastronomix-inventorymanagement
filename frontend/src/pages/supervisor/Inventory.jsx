...
const Inventory = () => {
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      ...
      try {
        ...
        // Calculate FIFO average cost with GST included
        ...
        // Get latest GST-inclusive price
  ...
  const filteredInventory = inventory.filter(item => {
    ...
    if (!stockLevelFilter.includes('all')) {
      const checks = []
      if (stockLevelFilter.includes('low')) checks.push(item.quantity <= item.low_stock_threshold)
      if (stockLevelFilter.includes('out')) checks.push(item.quantity === 0)
      if (stockLevelFilter.includes('in')) checks.push(item.quantity > item.low_stock_threshold)
  ...
  const exportToCSV = () => {
    const session = getSession()
    const headers = ['Material Name', 'Last Price', 'Code', 'Category', 'Quantity', 'Unit', 'Low Stock Threshold', 'Status', 'Avg Cost per Unit', 'Total Value (FIFO)']
  ...
  const exportToExcel = () => {
    ...
    const inventoryData = [
      ['Material Name', 'Last Price', 'Code', 'Category', 'Quantity', 'Unit', 'Low Stock Threshold', 'Status', 'Avg Cost per Unit', 'Total Value (FIFO)'],
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      autoTable(doc, {
        startY: yPos,
        head: [['Material Name', 'Last Price', 'Code', 'Category', 'Quantity', 'Low Stock Threshold', 'Status', 'Avg Cost/Unit', 'Total Value (FIFO)']],
        body: tableData,
        theme: 'striped',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-card border-2 border-border rounded-xl p-3 lg:p-4 mb-4 lg:mb-6">
          <div className="flex flex-col gap-2 lg:flex-row lg:gap-3">
            ...
            />
            {(!categoryFilter.includes('all') || !stockLevelFilter.includes('all')) && (
              <button
                type="button"
                onClick={() => {
